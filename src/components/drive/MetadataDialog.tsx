      ...
      <DialogContent className="sm:max-w-[420px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {item.type === "directory"
              ? "Folder details"
              : metadata?.file?.mime || "File details"}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3">
              ...
              <div className="bg-white/5 border border-white/5 rounded-2xl p-4 space-y-3">
                ...
                <div className="flex flex-col gap-2">
                  {faces.map((face, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between p-2 rounded-xl bg-white/5 border border-white/5 transition-all hover:bg-white/10 group"
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={cn(
                        ...
                        >
                          {face.user
                            ? `${face.user.firstName[0]}${face.user.lastName[0]}`
                            : "?"}
                        </div>
                        <div>
                      ...
                      <div className="text-right">
                        <p className="text-[8px] font-black text-zinc-600 uppercase tracking-tighter">
                          Confidence
                        </p>
            ...
            {/* Emotions & Sentiment */}
            {metadata?.analysis?.emotions &&
              metadata.analysis.emotions.length > 0 && (
                <div className="bg-white/5 border border-white/5 rounded-2xl p-4 space-y-3">
                  <div className="flex items-center gap-2 mb-1">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {metadata.analysis.emotions.map((e, i) => (
                      <div
                        key={i}
                        className="px-3 py-1.5 rounded-xl bg-white/5 border border-white/5 flex items-center gap-2"
                      >
            ...
            {/* Object Detection */}
            {metadata?.analysis?.objects &&
              metadata.analysis.objects.length > 0 && (
                <div className="bg-white/5 border border-white/5 rounded-2xl p-4 space-y-3">
                  <div className="flex items-center gap-2 mb-1">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {metadata.analysis.objects.slice(0, 8).map((obj, i) => (
                      <span
                        key={i}
                        className="px-2 py-1 rounded-lg bg-blue-400/5 border border-blue-400/10 text-blue-400/80 text-[9px] font-bold capitalize"
                      >
                        {obj.label}
                      </span>
                    ))}
          ...
          {tags.length > 0 && (
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {visibleTags.map(
                  (t: { tag: string; confidence?: number }, i: number) => (
                    <span
                      key={i}
                      className="rounded-full border border-border bg-muted/40 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-tight flex items-center gap-1.5"
                    >
                      {t.tag}
                      {t.confidence && (
                ...
                {hasMoreTags && !showAllTags && (
                  <button
                    ...
                    className="rounded-full border border-primary/20 bg-primary/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-tight text-primary"
                  >
                    +{tags.length - 4} More
                  </button>
                )}
                {showAllTags && hasMoreTags && (
                  <button
                    ...
                    className="rounded-full border border-border bg-muted/40 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-tight text-muted-foreground"
                  >
                    Show Less
                  </button>
                )}
