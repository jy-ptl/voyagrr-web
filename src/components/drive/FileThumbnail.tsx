...
export const FileThumbnail = ({
  ...
  const [thumbnailUrl, setThumbnailUrl] = useState<string | null>(
    type === "file" ? thumbnailCache.get(fileId) || null : null,
  );
  const [loading, setLoading] = useState(
    type === "file" && !thumbnailCache.has(fileId),
  );
  ...
  if (type === "directory") {
    return (
      <div
        className={cn(
          "bg-primary/10 text-primary flex items-center justify-center rounded-lg transition-all duration-300",
          className,
        )}
      >
  ...
  if (loading) {
    return (
      <div
        className={cn(
          "bg-zinc-800 animate-pulse flex items-center justify-center rounded-lg",
          className,
        )}
      >
        <ImageIcon
          className={cn("h-6 w-6 text-zinc-600", iconClassName)}
          strokeWidth={1.5}
        />
      </div>
    );
  ...
  if (error || !thumbnailUrl) {
    return (
      <div
        className={cn(
          "bg-zinc-800 text-zinc-500 flex items-center justify-center rounded-lg group-hover:bg-zinc-700 group-hover:text-white transition-all duration-300",
          className,
        )}
      >
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-lg bg-zinc-900 border border-white/5",
        className,
      )}
    >
      {fit === "contain" ? (
        <>
          <div
            className="absolute inset-0 scale-110 bg-cover bg-center blur-2xl opacity-35 transition-transform duration-500 group-hover:scale-[1.2]"
          ...
          <img
            src={thumbnailUrl}
            alt="File thumbnail"
            className="relative h-full w-full object-contain p-1.5 transition-transform duration-500 group-hover:scale-[1.04]"
